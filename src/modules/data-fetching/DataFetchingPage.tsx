...
function parseGames(snapshot: any): ParsedGame[] {
  const list = snapshot?.responseBody?.[0]?.data?.mainEventList?.competitions || [];
  ...
  for (const competition of list) {
    for (const event of competition.events || []) {
      out.push({
        ...
        sportName: snapshot?.responseBody?.[0]?.data?.mainEventList?.sportName || "Unknown",
        marketsCount: (event.collections || []).reduce((n: number, c: any) => n + (c.markets?.length || 0), 0)
      });
    }
  }
  return out;
...
export function DataFetchingPage() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-3xl font-display font-bold text-white uppercase italic">External Data <span className="text-brand">Control</span></h1>
      ...
      <Card className="bg-[#1A1A1A] border-zinc-800">
        <CardHeader>
          ...
          <CardDescription>
            Snapshot time: {oddsLatest.data?.snapshot?.fetchedAt || "No snapshot yet"} | Catalog snapshot: {catalogLatest.data?.snapshot?.fetchedAt || "No snapshot yet"}
          </CardDescription>
        </CardHeader>
        <CardContent>
